...
from .blocks import TabbedPanel
...
from .settings import CUSTOM_SIMPLE_PANELS, CUSTOM_COMPOSED_PANELS, TRANSLATE_SLUGS
...
class WagtailTranslator(object):
    ...
    def _patch_other_models(self, model):
        if hasattr(model, 'edit_handler'):
            ...
            for field in model._meta.get_fields():
                if field.name in translation_registered_fields:
                    for language in mt_settings.AVAILABLE_LANGUAGES:
                        translated_field = copy.deepcopy(field)
                        translated_field.name = build_localized_fieldname(field.name, language)
            model.panels = self._patch_panels(model.panels)
        else:
            panels = extract_panel_definitions_from_model_class(model)
            str_lang = []
            for lang in mt_settings.AVAILABLE_LANGUAGES:
                # build a list with lang name extesions "_en", "_ca" ...
                str_lang.append(str('_%s' % (lang.replace('-', '_'))))
            # discard localized fields to build tab panel only for multilingual fields
            panels = filter(lambda x: not x.field_name.endswith(tuple(str_lang)), panels)
            panels = self._patch_panels(panels, model)
    ...
    def _patch_panels(self, panels_list, related_model=None):
        ...
        for panel in panels_list:
            if panel.__class__ in SIMPLE_PANEL_CLASSES:
                ...
                pass  # TODO patched_panels.append(self._patch_inline_panel(panel))
            else:
    ...
    def _patch_simple_panel(self, model, original_panel):
        ...
        for language in mt_settings.AVAILABLE_LANGUAGES:
            ...
            if not original_field.blank and language == mt_settings.DEFAULT_LANGUAGE:
                ...
                tabbed_panel_class = "stream-field"
        ...
        return [TabbedPanel(translated_panels, classname=tabbed_panel_class)]
    ...
    def _patch_inline_panel(self, panel):
        ...
        try:
            ...
            # TODO pass
            self._patch_other_models(related_model)
        else:
...
def patch_wagtail_models():
    ...
    for model_class in registered_models:
        if issubclass(model_class, Page) or issubclass(model_class, Model) or model_class in get_snippet_models() or issubclass(model_class, BaseSetting):
